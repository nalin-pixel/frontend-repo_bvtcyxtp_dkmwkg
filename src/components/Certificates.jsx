    ...
    <section id="certificates" className="relative py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-white/70 mt-2 max-w-2xl">Some of the learning milestones that shaped my craft.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((c) => (
            <article key={c.title} className="group relative rounded-xl border border-white/10 bg-white/5 p-5 overflow-hidden">
              ...
              <span className="inline-flex mt-4 w-fit rounded-md bg-white/10 px-2 py-1 text-xs text-white/70">{c.year}</span>
            </article>
          ))}
        </div>
      </div>
